...
void Can_Tx_Command(void *pvParameters) {
    while (1) {
        ...
        if ((HAL_CAN_GetTxMailboxesFreeLevel(&hcan1) != 0) && !tx_in_flight && !F103_Status) {
            Ctrl_Frame command = {0};
            xQueueReceive(queue_ctrl_rpm_command, &command, portMAX_DELAY);
            if (command.CommandHeader.StdId == 0x301) {   // 控制命令
                if (command.data[0] == 0x01) {  // 设置转速
                    uint32_t pTxMailboxNum = 0;
                    if (HAL_CAN_AddTxMessage(&hcan1, &command.CommandHeader, command.data, &pTxMailboxNum) != HAL_OK) {
...
void app(void) {
    ...
    queue_feedback_rpm = xQueueCreate(8, sizeof(CAN_Frame));
    queue_ctrl_rpm_command = xQueueCreate(1, sizeof(Ctrl_Frame));
